-- ============================================================
-- BOLICASH - ESQUEMA COMPLETO Y CONSOLIDADO
-- Tablas: contactos, trivias, participaciones
-- RLS + RPC publico + RPCs admin + Edición sin imágenes
-- ============================================================
...
create index if not exists participaciones_trivia_id_idx
  on public.participaciones (trivia_id);

create index if not exists participaciones_ganadores_idx
...
-- Lectura pública de trivias publicadas
drop policy if exists "trivias_lectura_publica" on public.trivias;
create policy "trivias_lectura_publica" on public.trivias
  for select to anon, authenticated
  using (publicada = true);

-- Actualización directa por admin
...
-- ---------- 4. RPC PÚBLICO: REGISTRAR PARTICIPACIÓN ----------
...
-- Actualizar dinámica (Edición completa)
...
grant execute on function public.crear_trivia_admin to authenticated, anon;
grant execute on function public.actualizar_trivia_admin to authenticated, anon;
grant execute on function public.publicar_trivia_admin to authenticated, anon;
grant execute on function public.cambiar_estado_admin to authenticated, anon;
grant execute on function public.guardar_resultado_admin to authenticated, anon;
grant execute on function public.eliminar_trivia_admin to authenticated, anon;
grant execute on function public.buscar_ganadores_admin to authenticated, anon;
grant execute on function public.listar_contactos_admin to authenticated, anon;
grant execute on function public.listar_participaciones_admin to authenticated, anon;
grant execute on function public.estadisticas_admin to authenticated, anon;
